...
echo "== 4/6 pure-ASCII =="
bad=$(LC_ALL=C grep -rlP '[^\x00-\x7F]' src tests tools scripts results README.md claims.toml REVIEW.md PREREG.md bench_results docs 2>/dev/null || true)
[ -z "$bad" ] || { echo "$bad"; fail ascii; }; echo "   ok"
...
leak=$(grep -rniIE --exclude-dir=__pycache__ --exclude-dir=.venv --exclude=.leakpatterns "$pat" src tests tools scripts results bench_results README.md claims.toml REVIEW.md PREREG.md reproduce.sh docs 2>/dev/null || true)
[ -z "$leak" ] || { echo "$leak"; fail "leak"; }; echo "   ok"
